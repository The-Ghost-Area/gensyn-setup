...
# Safer: check if `tput` is available and terminal supports colors
...
get_random_color() {
    ...
    # Avoid error if tput is not available
    if [ -n "$GREEN" ]; then
        echo "$(tput setaf ${colors[$RANDROM % ${#colors[@]}]})"
    else
        echo ""
    fi
}
...
print_banner() {
    # Use tput to move cursor to top-left instead of `clear` to prevent flicker
    tput cup 0 0
